...
export const STR = {
  ...
  "this node could not execute the tools that bring an interface up: the kernel refused to start them (\"Operation not permitted\"), so the bring-up never began. This is not a port or subnet conflict. This node's installation is managed declaratively, so this belongs in the system configuration that declares it rather than in a command here: the node daemon must not set NoNewPrivileges, and the security policy and capability set that configuration gives this node have to let the WireGuard tools execute. Restart the node daemon once the change is in — a rebuild does not restart it on its own, so the old unit keeps running.":
    "эта нода не смогла запустить инструменты, которыми поднимается интерфейс: ядро отказалось их стартовать («Operation not permitted»), поэтому подъём даже не начался. Это не конфликт порта или подсети. Установка этой ноды описана декларативно, поэтому это относится к конфигурации системы, которая её описывает, а не к команде здесь: демон ноды не должен выставлять NoNewPrivileges, а политика безопасности и набор capabilities, которые эта конфигурация даёт ноде, должны позволять запускать инструменты WireGuard. Перезапустите демон ноды, когда изменение внесено, — пересборка сама его не перезапускает, и продолжает работать старый юнит.",
  ...
  "this node could not execute the tools that bring an interface up: the kernel refused to start them (\"Operation not permitted\"), so the bring-up never began. This is not a port or subnet conflict, and the datapath module is not the problem. Either those binaries carry file capabilities that this node's capability bounding set no longer covers, or an exec-control layer (SELinux, fapolicyd, or an AppArmor policy this node cannot read) is denying them. Run on the node:  getcap $(command -v ip) $(command -v wg) $(command -v awg)  for the first, and  dmesg | grep -i 'denied'  for the second — one of them names it. Once the tools run again, this interface creates itself on the next sync.":
    "эта нода не смогла запустить инструменты, которыми поднимается интерфейс: ядро отказалось их стартовать («Operation not permitted»), поэтому подъём даже не начался. Это не конфликт порта или подсети, и модуль датапаса тут ни при чём. Либо на этих бинарниках стоят файловые capabilities, которых больше нет в capability bounding set этой ноды, либо их запуск запрещает слой контроля запуска (SELinux, fapolicyd или политика AppArmor, которую нода не может прочитать). Выполните на ноде:  getcap $(command -v ip) $(command -v wg) $(command -v awg)  для первого и  dmesg | grep -i 'denied'  для второго — что-то из них назовёт причину. Как только инструменты снова запускаются, интерфейс создастся сам на следующей синхронизации.",
  "the interface tools ran on this node, but the kernel refused the operation they asked for (\"RTNETLINK answers: Operation not permitted\"). This is not a port or subnet conflict: the tools are being denied the network privilege they need. In a container that means the node is missing NET_ADMIN (in docker-compose.yml: cap_add: [NET_ADMIN], then docker compose up -d). On a host it means a security policy is confining them —  dmesg | grep -i 'denied'  names which one. Then this interface creates itself on the next sync.":
    "инструменты интерфейса на этой ноде запустились, но ядро отказало в операции, которую они запросили («RTNETLINK answers: Operation not permitted»). Это не конфликт порта или подсети: инструментам отказывают в сетевой привилегии, которая им нужна. В контейнере это значит, что ноде не хватает NET_ADMIN (в docker-compose.yml: cap_add: [NET_ADMIN], затем docker compose up -d). На хосте это значит, что их ограничивает политика безопасности —  dmesg | grep -i 'denied'  покажет какая. После этого интерфейс создастся сам на следующей синхронизации.",
  "this node's container could not execute the tools that bring an interface up: the kernel refused to start them (\"Operation not permitted\"), so the bring-up never began. This is not a port or subnet conflict. A container gets this when its capability set no longer covers what those binaries ask for — give the node NET_ADMIN (in docker-compose.yml: cap_add: [NET_ADMIN]) and recreate it (docker compose up -d), then this interface creates itself on the next sync.":
    "контейнер этой ноды не смог запустить инструменты, которыми поднимается интерфейс: ядро отказалось их стартовать («Operation not permitted»), поэтому подъём даже не начался. Это не конфликт порта или подсети. У контейнера так бывает, когда его набор capabilities больше не покрывает то, что запрашивают эти бинарники, — выдайте ноде NET_ADMIN (в docker-compose.yml: cap_add: [NET_ADMIN]) и пересоздайте её (docker compose up -d), после чего интерфейс создастся сам на следующей синхронизации.",
