...
export const STR = {
  ...
  "A date is the last day they can reach these networks; leave it empty for no end. Only their WireGuard and AmneziaWG devices on the same node count — a turn-server device can't reach a restricted network.": "Дата — последний день, когда эти сети доступны; оставьте пустой, чтобы без срока. Считаются только их устройства WireGuard и AmneziaWG на той же ноде — устройство за turn-сервером не достучится до сети с ограниченным доступом.",
  ...
  "until {date}": "до {date}",
  "no end": "без срока",
  "{devices} here": "здесь: {devices}",
  "no device on {node}": "нет устройств на {node}",
  "{name}: turn-server devices can't reach a restricted network ({devices}).": "{name}: устройства за turn-сервером не достучатся до сети с ограниченным доступом ({devices}).",
  "Access has ended for {names}.": "Доступ закончился: {names}.",
  ...
  // budget-ok: wraps
  "Every client on {node} can reach this: {peers} belonging to {users}. Choose who under “Who can reach these networks”.":
    "Сюда попадёт любой клиент на {node}: {peers} у {users}. Выбрать, кому, — в «Кому доступны эти сети».",
