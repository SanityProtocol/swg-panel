...
export const STR = {
  ...
  "UDP, separate from the listen and internal WG ports. The node opens it and routes the RAW subnet.": "UDP, отдельно от порта прослушивания и внутреннего WG-порта. Узел откроет его и настроит маршрутизацию RAW-подсети.",
  ...
  "The qWDTT app can't read this from a link yet, so each user sets it once: *Settings → server raw port = {v1}*, then switch the connection mode to *raw*. Their existing link keeps working for WireGuard mode.": "Приложение qWDTT пока не читает это из ссылки, поэтому пользователь задаёт порт один раз: *Настройки → RAW-порт сервера = {v1}*, затем переключает режим подключения на *raw*. Прежняя ссылка продолжает работать в режиме WireGuard.",
  "RAW mode is on for this server (port {v1}). The link can't carry it — the user sets that port in the app and switches the mode to RAW.": "На этом сервере включён режим RAW (порт {v1}). Ссылка его не передаёт — пользователь указывает порт в приложении и переключает режим на RAW.",
  "row|kind": "тип",
  "row|endpoint": "адрес",
