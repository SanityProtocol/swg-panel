...
# ───────────────────────── write .env ─────────────────────────
mkdir -p "$PREFIX$INSTALL_DIR"
_UPD_TRIG=""; case "$PROFILE" in host|master) _UPD_TRIG=/var/lib/swg-panel/.update-request;; esac   # panel writes this on a one-click update; the host swg-update.path (wired after compose up) watches it
...
wire_host_updater(){
  case "$PROFILE" in host|master) :;; *) return 0;; esac
  if $DRYRUN; then echo "    [skip] wire host one-click updater (swg-update.path → data/lib/.update-request)"; return 0; fi
  local trig="$INSTALL_DIR/data/lib/.update-request"
  cat > /usr/local/bin/swg-update <<'WRAP'
#!/usr/bin/env bash
# swg-update — root entrypoint for the panel's one-click update. swg programs only (--no-components skips docker
# engine / wg-awg / turn-proxies); on a docker box this is `compose pull && up` for the swg images only.
set -euo pipefail
...
EOF
  # the unprivileged panel writes the trigger into its OWN data/lib, so create it INSIDE the container with the
  # right owner — a root-created host file would be unwritable by the panel. Only when missing (re-touch = spurious run).
  [ -e "$trig" ] || docker exec swg-panel sh -c ': > /var/lib/swg-panel/.update-request' 2>/dev/null || true
  systemctl daemon-reload 2>/dev/null || true
  systemctl enable --now swg-update.path >/dev/null 2>&1 || warn "couldn't enable swg-update.path"
  ok "one-click update wired — the panel's $(b Update) button runs a swg-only update on the host"
}
wire_host_updater
