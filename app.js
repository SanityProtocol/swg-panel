...
function NodeCard({ n }) {
  ...
  return html`<div class=${"ncard" + (removing ? " removing" : "")}>
    <div class="ncard-body clk" onClick=${() => go("#/node/" + encodeURIComponent(n.id))}>
      ...
      <div class="nmeta">
        <span class="nm-item"><span class="nm-l">Peers</span>${here.length ? html`<span class="nm-v">${onl}<small>/${here.length}</small></span>` : html`<span class="nm-v faint">none</span>`}</span>
        <span class="nm-item"><span class="nm-l">IF</span>${ifTags.length ? html`<span class="tags">${ifTags}</span>` : html`<span class="nm-v faint">—</span>`}</span>
        <span class="nm-item"><span class="nm-l">Turn</span>${tps.length ? html`<span class="tags">${tps.map(tp => html`<span class="tg tg-turn">${turnLabel(tp.service, portOf(tp.listen) || portOf(tp.connect))}</span>`)}</span>` : html`<span class="nm-v faint">—</span>`}</span>
