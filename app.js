    ...
    ${(nrec.mesh_peers || []).length ? html`<${Panel} icon="network" title="Node connections" tone="pending" count=${(nrec.mesh_peers || []).length}
      ...
      <div class="ifgrid">${[...(nrec.mesh_peers || [])].sort((a, b) => Store.nodeName(a.peer).localeCompare(Store.nodeName(b.peer))).map(mp => {
        ...
        return html`<div key=${peer} class=${"ifcard tp" + (reprov ? "" : " clickable") + (muted ? " down" : "")} onClick=${reprov || !ifn ? null : () => openConnectionEdit(name, ifn)}>
          <div class="ifcard-top"><span class="iftype turn" style=${"--tfc:" + col}><${Ic} i="server"/></span><span class="ifname">${Store.nodeName(peer)}</span><span class="grow"></span>${carried.length ? html`<span class="egb egb-cascade" style="margin-top:-2px" title=${"Cascade: relays " + carried.length + " interface" + (carried.length === 1 ? "" : "s") + " out via " + Store.nodeName(peer)}><${Ic} i="cascade"/>cascade</span>` : null}${smartCarried.length ? html`<span class="egb egb-smart" style="margin-top:-2px" title=${"Smart cascade: routes selected destinations out via " + Store.nodeName(peer)}><${Ic} i="smart"/>smart cascade</span>` : null}${reprov ? html`<span class="tg tg-busy" title="Rebuilding this node's mesh link — it reconnects in a few seconds"><${Ic} i="clock"/>re-provisioning</span>` : html`<span class=${"lkdot " + lk} style="margin-top:-2px" title=${lkTitle}></span>`}</div>
          <div class="ifcard-rows">
...
function ConnectionEditSheet({ node, iface }) {
  ...
  return html`<${Sheet} title=${"Connection to " + Store.nodeName(peer)} width=${680} onClose=${closeModal}
    ...
    ${(carried.length || smartCarried.length) ? html`<div style="margin-top:16px">
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:9px;font-size:12px;font-weight:600;color:var(--ink)">Forwarding${carried.length ? html`<span class="egb egb-cascade"><${Ic} i="cascade"/>cascade</span>` : null}${smartCarried.length ? html`<span class="egb egb-smart"><${Ic} i="smart"/>smart cascade</span>` : null}</div>
      ${carried.length ? html`<div class="fwd-list">${carried.map(c => html`<div class="fwd-row">${ifBadge(c.iface)}<span class="fwd-arr">→</span>${peerNm}${c.ip ? html`<span class="faint">as ${c.ip}</span>` : ""}</div>`)}</div>` : null}
      ${smartCarried.length ? html`<div class="fwd-list" style=${carried.length ? "margin-top:6px" : ""}>${smartCarried.map(s => html`<div class="fwd-row">${ifBadge(s.iface)}<span class="fwd-arr">→</span>${peerNm} <span class="faint">(${s.cats.map(c => SMART_CAT_LABEL[c] || c).join(", ")})</span></div>`)}</div>` : null}
