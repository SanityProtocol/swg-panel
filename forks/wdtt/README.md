...
- Build label: **`1.2.4-3`** — our third build of upstream 1.2.4 (same pin; adds the owner-device fix below).
  Published as `wdtt-amurcanov-1.2.4-3` (2026-09-17), amd64 + arm64, and rig-proven on the published amd64 bytes;
  `1.2.4-2` stays published as the rollback target.
- Static (`CGO_ENABLED=0`), ~8 MB. Upstream `go.mod` says Go 1.25; the published build used Go 1.27.1.
