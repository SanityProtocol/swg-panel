...
- **Server box gone for good?** When the old machine is truly lost — nothing left to recover from — the
  ...
  every config and QR already handed out keeps working. It shows exactly what the rebuild will change
  before the token rotates. Open the node in the panel and rebuild it.
