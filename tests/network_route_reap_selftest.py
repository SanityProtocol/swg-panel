...
§4.12) and the ACL filter in front of it.
 ...
 [11] a network handed from a device on one interface to a device on another moves in ONE pass: the old route goes before
      the new install, which would otherwise meet it, refuse `route_exists` and leave the network dead for a routing pass.
...
Run: python3 tests/network_route_reap_selftest.py            (0 = pass)
     ...
     --perturb-handover   drops the handover pass and expects RED on [11].
"""
...
PERTURB_HANDOVER = "--perturb-handover" in sys.argv
...
if PERTURB_HANDOVER:
    ...
    _cut = "        for p in sorted(gone):\n            run([\"ip\", \"route\", \"del\", p, \"dev\", iface])\n            res[\"changed\"] += 1\n        live[iface] = set(ps) - gone\n"
    ...
    open(NODED, "w", encoding="utf-8").write(_src.replace(_cut, "        pass\n", 1))
...
print("\n[11] a handover moves in one pass")
_moved = with_net("192.168.60.0/24", iface="awg1")
_moved["wg0"] = [dict(p) for p in INERT["wg0"]]
_moved["awg1"][0]["allowed_ips"] = INERT["awg1"][0]["allowed_ips"]
_moved["wg0"][0]["allowed_ips"] = INERT["wg0"][0]["allowed_ips"] + ",192.168.60.0/24"
# the kernel: a replace onto another device is refused while the old route is still in `main` (net_route_install's guard)
def _install(p, dev, table="", _guard=True, _verify=True):
    REC["install"].append((p, dev))
    old = ["ip", "route", "del", p, "dev", "awg1"] not in REC["run"]
    REC["run"].append(["install", p, dev])            # into the same log, so the ORDER is what is checked
    return (False, "route_exists", "awg1") if old else (True, None, "")
setup(acl=acl_for(_moved, "wg0"))
N.net_route_install = _install
kept, res = rec(_moved, extra=[("awg1", "192.168.60.0/24")])
_order = [c for c in REC["run"] if "192.168.60.0/24" in c]
check("⚠️ the old route on the other interface is deleted, and before anything is installed",
      _order[:1] == [["ip", "route", "del", "192.168.60.0/24", "dev", "awg1"]], REC["run"])
check("⚠️ …so the new device carries it in this same pass — not refused `route_exists`",
      ("192.168.60.0/24", "wg0") in kept and not res["errors"], (kept, res))
check("…and it is not deleted twice", len([c for c in REC["run"] if c[:3] == ["ip", "route", "del"]]) == 1, REC["run"])
setup()
kept, res = rec(INERT, extra=[("awg1", "192.168.60.0/24")])
check("a route nobody wants anywhere is still reaped at the end, as before", ["ip", "route", "del", "192.168.60.0/24", "dev", "awg1"] in REC["run"])
