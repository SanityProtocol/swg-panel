...
if P_GUARD:
    relay = cut(relay, 'if is_local_addr(dst[0]):', 'if False:', "relay guard")
if P_NFT:
...
check("…and refuses a LOCAL address on any port — never a bare port test",
      re.search(r"if is_local_addr\(dst\[0\]\):", acc) is not None and "a.port" not in acc,
      "a port test breaks a client dialling a remote host on that port, and a local destination on another port "
      "came back through the divert (7,790 connections from one connect, measured 2026-09-16)")
...
check("⚠⚠ …and a leg disarmed BESIDE an armed one has its port shut as well",
      ...
      and "_relay_nft(entries, guard_ports, links)" in noded,
...
check("…and the builder emits that counter per instance",
      'tcp %scounter comment "%s"' in noded and "_relay_syn_tag(" in noded)
check("…with no verdict, so it falls through to the divert",
      'tcp %scounter comment "%s"' in noded and "accept comment" not in
