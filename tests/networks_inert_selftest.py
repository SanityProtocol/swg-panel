...
check("…and the reply names `net_share` only behind that result, computed only when a peer restricts one",
      ...
      and 'q.get("routes") and net_restricted(q)' in _psrc)
# ⚠️ "Restricted" is net_restricted(), not "has a stored share" — a PRIVATE provider restricts one too, and asking the
# narrower question here meant the whole net_share pass was skipped for it and its networks went to the entire node.
# The inert promise still holds in the direction this section is about: nothing private, nothing shared ⇒ nothing computed.
check("⚠️ …and a private provider with no stored share DOES count as restricting one",
      P.net_restricted({"private": True, "routes": ["192.168.1.0/24"]}) is True
      and P.net_restricted({"routes": ["192.168.1.0/24"]}) is False)
