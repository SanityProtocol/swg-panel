...
> - **Принятие сервера WDTT больше не опустошает его** (1.6.1) — приём сервера, который эта машина запускала раньше, тихо удалял всех пользователей. Теперь пользователи сервера передаются в панель и импортируются; чтобы удалить пользователя, нужно его удалить.
> - **Панель читает собственный сертификат** (1.6.1) — она могла всю жизнь машины показывать «самоподписанный», обслуживая настоящий. Теперь это исправляется само.
> - **Исправления** — узел с закреплённым исходящим адресом больше не сообщает об ошибке каждые несколько секунд, предпросмотр цвета в настройках перестал быть прозрачным, «Аутентификация» снова одна панель настроек.
<!-- WHATS-NEW:END -->
