...
- **Серверы WDTT.** Целый VPN-сервер, спрятанный внутри потока, похожего на видеозвонок VK. Turn-прокси
  ...
  другой интерфейс, выбирайте один из четырёх форков и выдавайте людям пароль вместо ключа. Его конфигурация
  появляется на странице подписки рядом со всем остальным, вместе со ссылкой на приложение.
- **Возьмите то, что уже есть.** Уже подняли WireGuard, AmneziaWG или WDTT на машине вручную? Панель найдёт эти
...
> [Техническое руководство](README.technical.md#quick-start).
...
Если на этом сервере **уже работали** WireGuard, AmneziaWG или WDTT, они появятся на его странице в панели
...
4. На сервере **WDTT** сканировать нечего: человек получает ссылку, которая открывается прямо в приложении
   WDTT (или QR — для приложений, которые его сканируют), а панель подсказывает нужное приложение для его
