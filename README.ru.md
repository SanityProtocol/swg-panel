...
> - **Серверы WDTT** — третий вид сервера, управляемый из панели: он несёт туннель WireGuard внутри потока, похожего на видеозвонок VK, поэтому на проводе не выглядит как VPN. Четыре форка на выбор, пользователи — как все остальные, конфигурация — на странице подписки.
> - **Приём уже существующих серверов** — панель находит серверы WireGuard, AmneziaWG и WDTT, которые машина уже держала (в том числе остановленные), и забирает их как есть: те же ключи, те же пиры, никому не нужно ничего переимпортировать.
> - **Установщики больше не спрашивают про настройку VPN** — они поднимают службу и на этом останавливаются; интерфейсы, turn-прокси и серверы WDTT создаются в панели, где видно, что вы делаете.
<!-- WHATS-NEW:END -->
