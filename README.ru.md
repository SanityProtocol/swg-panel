...
> - **Исправление безопасности для установок в Docker** — запрос из собственной очереди панели мог выполнить команды с правами root на хосте. Оболочка для этих вызовов больше не используется, а очередь проверяется до того, как что-либо будет выполнено. Сообщил [@anupamme](https://github.com/anupamme).
> - **«Не предлагать» сервер на выбранной ОС** — можно выбрать «ничего» вместо приложения, которое не подключится, и сервер просто не появится на страницах подписки этих пользователей.
<!-- WHATS-NEW:END -->
