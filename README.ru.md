...
- [Шаг 3 — Добавьте людей и раздайте доступ](#шаг-3--добавьте-людей-и-раздайте-доступ)
...
- **Одна страница, чтобы управлять всем.** Добавляйте серверы, добавляйте людей, раздавайте доступ — всё из веб-панели.
...
- **Больше одного сервера.** Разместите серверы в разных странах; человек может переключаться между ними при сбое.
...
| | |
|---|---|
| **Люди** — все, кому вы выдали доступ, одним взглядом | ![Пиры](screenshots/peers.png) |
...
- **Публичный IP-адрес** (чтобы ваши люди могли до него дотянуться).
...
## Шаг 3 — Добавьте людей и раздайте доступ
...
- **Добавляйте и удаляйте людей в любой момент.** Изменения доходят до серверов за считанные секунды. Удалите
  кого-то — и его доступ прекратится при следующей проверке связи.
- **Меняйте вход в панель** в **⚙︎ → Account** (Аккаунт) — вступает в силу немедленно.
...
на месте, сохранив все ваши настройки и людей:
```
...
- **Ручные (всё равно рекомендуется).** Для копии вне сервера сохраните куда-нибудь в надёжное место папку
  состояния панели (`/var/lib/swg-panel`, где лежат ваши люди и серверы) — одной копии достаточно, чтобы
...
- **Повторный запуск любого установщика безопасен.** Он замечает, что установка уже есть, и **сохраняет ваши
  данные** (вход, сертификат, людей, серверы) — удобно, если команда прервалась или чтобы поменять опцию.
...
- **Потеряли всю машину с панелью?** Верните сохранённую папку `/var/lib/swg-panel` на свежую установку — и
  ваши люди и серверы снова на месте; серверы переподключаются сами.
...
секунд, и ваши люди едва его заметят. (Машину-«мастер» можно преобразовать целиком, либо только её половину с
...
turn-прокси) — ничего не удаляется без вашего согласия, и вы можете сохранить данные людей/серверов для
