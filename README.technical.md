...
- > **Upgrading a Docker panel from before 1.2.12-beta?** The `swg-sub` service entered `docker-compose.yml` in that release, and **an update never restages the compose file** — so an older Docker panel keeps a compose with no `swg-sub` and Settings → Subscriptions has nothing to bind. Re-run the Docker installer (it re-copies `docker-compose.yml`) or paste the `swg-sub` block in by hand. On bare-metal, `update.sh`'s `ensure_sub_server` installs it for you.
