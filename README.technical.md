...
The installers refuse to run here, and that is the feature: they write into `/opt` and
`/etc/systemd/system`, which `nixos-rebuild` neither manages nor sees, so a run that *appeared* to
work would leave an install your configuration cannot update or remove. Import the flake and declare
the module instead. Both delivery methods are first-class — a container arm running these same
images, and a native arm running the programs from your own store.
...
`services.swg-node` is the same shape, and **Nodes → Add node** prints the whole block beside the
enrolment token. Every option, the master role, adoption of a box you already run, and the one
support boundary worth reading first: [`nix/README.md`](nix/README.md).
...
- > **Upgrading a Docker panel from before 1.2.12-beta?** The `swg-sub` service entered `docker-compose.yml` in that release, and **an update never restages the compose file** — so an older Docker panel keeps a compose with no `swg-sub` and Settings → Subscriptions has nothing to bind. Re-run the Docker installer (it re-copies `docker-compose.yml`) or paste the `swg-sub` block in by hand. An update now *tells* you when your compose is older than the release and names what is missing. On bare-metal, `update.sh`'s `ensure_sub_server` installs it for you.
...
**Re-running an installer is safe** — it detects an existing install, keeps your `.env` + `./data` (token, login, certificate, interfaces), and offers the current values as defaults; the wg/awg and turn-proxy steps show what's already there and let you add more. To start fresh, run the uninstaller first. (Docker: re-apply with `PULL_POLICY=always … bootstrap.sh docker host|node` so the new image + compose land.) **One exception, on Docker:** re-running restages `docker-compose.yml` from the release, and the only per-interface UDP ports it publishes are the bootstrap one — so if you run the node on **bridge** networking and published extra ports by hand, re-add those lines afterwards. Host networking (the default) publishes nothing per interface and is unaffected.
